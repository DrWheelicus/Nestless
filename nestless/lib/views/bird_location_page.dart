...
Map<String, dynamic> locationMap = {"Cochrane": LatLng(49.147492170525624, -81.0280137279443), 
...
class BirdLocationPage extends StatefulWidget {
  ...
  @override
  State<BirdLocationPage> createState() => _BirdLocationPageState(this.selectedBirds);
}
    ...
    return Scaffold(body: Stack(
          children: [Positioned.fill(
            child: Opacity(opacity: 0.5, 
              child: Container(color: Colors.black))), Padding(padding: const EdgeInsets.all(30.0), 
            child: Stack(
              children: [Positioned.fill(
                child: FlutterMap(options: MapOptions(zoom: 9.0, center: myLocation),
              ...
              ])),Positioned(bottom: 25, left: 25, child: Opacity(opacity: 0.9, 
              child: Container(decoration: const BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(10)), color: Colors.blue), height: 50, width: 125,
                child: Stack(
                  children: [const Positioned(top: 5, left: 5, child: Text("Selected Location:", style: TextStyle(color: Colors.white))), 
                   Positioned(bottom: 5, left: 5, child: Text(locationName, style: const TextStyle(color: Colors.white)))])))
              ), Positioned(top: 25, right: 15, child: ElevatedButton(onPressed:() {
                Navigator.pop(context);
              }, child: const Text("Close"))), 
              Positioned(bottom: 25, right: -35, child: ElevatedButton(onPressed:() async {
              Position userCoords = await _currentLocation();
              var userLocation = LatLng(userCoords.latitude, userCoords.longitude);
              setState(() {
                myLocation;//= userLocation;
                if (myLocation != LatLng(43.86567452537625, -79.19257823567597)){
                  markerList.removeLast();
                }
                markerList.add( //use this instead of circlelayeroptions
                  Marker(
                    point: myLocation,
                    builder: (BuildContext context) {
                      return IconButton(
